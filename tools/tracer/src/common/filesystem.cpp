...
void mkdir(const std::string &path, int mode) {
    ...
    check(::mkdir(path.c_str(), mode) == 0, "Cannot create directory");
}

void rmdir(const std::string &path) {
    check(::rmdir(path.c_str()) == 0, "Cannot remove directory");
}

void chdir(const std::string &path) {
    check(::chdir(path.c_str()) == 0, "Cannot change directory");
}

void remove(const std::string &path) {
    check(::unlink(path.c_str()) == 0, "Cannot remove file");
}
