...
parser = argparse.ArgumentParser(description="Generate line plots from benchmark/microbenchmark evaluation")
...
def gen_func_pie(benchmark_name, pdf, benchmark):
    ...
    ax.pie(percentages, labels=labels, autopct='%1.1f%%', startangle=90, counterclock=False)
...
def gen_ipc_comparison(benchmark_name, pdf, global_ipc, benchmark, microbenchmarks):
    ...
    ax = fig.subplots(1, 1)
    ax.set_title("%s bencmark & microbenchmark IPC comparison" % benchmark_name, size="x-large")
    ...
    box = ax.get_position()
    factor = 0.9
    ax.set_position([box.x0, box.y0 + box.height * (1 - factor), box.width, box.height * factor])
    ax.grid(True)
...
def gen_micro_pie(benchmark_name, pdf, benchmark, microbenchmarks):
    ...
    ax = fig.subplots(1, 1)
    ax.set_title("%s weight distribution per microbenchmark" % benchmark_name, size="x-large")
...
def gen_plots(benchmark_name, pdf, function_name, function, microbenchmarks, micro_path):
    fig = get_next_fig(pdf)
    ...
    x_data = range(0, len(benchmark_metrics), math.floor(len(benchmark_metrics)/num_data_points))

    marker = None if len(benchmark_metrics) > 10 else "D"

    ax_ipc.plot(x_data, [benchmark_metrics[i].ipc for i in x_data], label="Benchmark", linewidth=linewidth, linestyle="solid", zorder=10, marker=marker)
    ax_instructions.plot(x_data, [benchmark_metrics[i].instructions for i in x_data], label="Benchmark", linewidth=linewidth, linestyle="solid", zorder=10, marker=marker)

    for micro in microbenchmarks:
        label = "Invocation %d" % micro.invocation_id
        metrics = load_metrics(micro_path[micro.invocation_id])
        assert(len(metrics) > 0)
        ax_ipc.plot(x_data, [metrics[i].ipc for i in x_data], label=label, linewidth=linewidth, linestyle="dashed", zorder=1, marker=marker)
        ax_instructions.plot(x_data, [metrics[i].instructions for i in x_data], label=label, linewidth=linewidth, linestyle="dashed", zorder=1, marker=marker)

    ax_ipc.set_title("Instructions Per Clock")
    ax_instructions.set_title("Retired Instructions")

    fig.suptitle("%s function %s (weight: %0.2f%%)" % (benchmark_name, function_name, function.percentage_samples * 100.0), size="x-large")
    ...
    ax_ipc.set_position([box.x0, box.y0 + box.height * (1 - factor), box.width, box.height * factor])
    box = ax_instructions.get_position()
    ax_instructions.set_position([box.x0, box.y0 + box.height * (1 - factor), box.width, box.height * factor])
    #Make the legend
    ax_ipc.legend(handles, labels,  bbox_to_anchor=(0,-.05, 2.2,-0.15), loc=9, ncol=4)
...
def load_function_data(results_path, benchmark):
    with open(os.path.join(results_path, "functions.txt")) as file:
        ...
        for line in lines:
            ...
            cluster_info = ClusteringInformation.from_file(os.path.join(results_path, name + "_cluster.json"))
            invocations = load_invocations_from_file(os.path.join(results_path, name, "benchmark.csv"))
...
if __name__ == '__main__':
    ...
    for function in benchmark.functions:
        ...
        for file in os.scandir(function_results_path):
            if not file.name.startswith("benchmark") and not file.name.endswith("_global.csv"):
                match = regex.match(file.name)
                assert(match)
                invocation = int(match.group(1))
    ...
    with PdfPages('report.pdf') as pp:
        ...
        gen_ipc_comparison(benchmark_name, pp, metrics[0].ipc, benchmark, microbenchmarks)

        for function_id in benchmark.functions:
            function = benchmark.functions[function_id]
            gen_plots(benchmark_name, pp, function_id, function, [micro for micro in microbenchmarks if micro.function_id == function_id], micro_path[function_id])
